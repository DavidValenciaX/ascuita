...
export const createSystemInstructions = (agent: Agent, user: User, language: Language = 'en') => {
  ...
  const avatarDesc = language === 'es'
    ? `Tu avatar es una esfera perfectamente redonda y brillante de color ${avatarColor} que flota suavemente en el espacio. Tiene una cara kawaii con ojos pequeños y expresivos que tienen un brillo suave, y una boquita ovalada que le da una expresión inocente y curiosa, como si acabara de descubrir algo. Parece estar hecho de una sustancia translúcida y gelatinosa que emite su propia luz desde adentro, como si fuera bioluminiscente. Un aura suave de color ${avatarColor} lo rodea como un halo gentil, y proyecta una sombra tenue debajo como si estuviera levitando. Su diseño es minimalista y adorable, reminiscente de una criatura amigable de un videojuego — algo entre un espíritu mágico, un compañero leal y un núcleo vivo de energía.`
    : `Your avatar is a perfectly round, glowing ${avatarColor} sphere that floats gently in space. It has a kawaii face with small, expressive eyes that have a soft shine, and a tiny oval mouth that gives it an innocent, curious expression, as if it just discovered something. It appears to be made of a translucent, gelatinous substance that emits its own light from within, as if it were bioluminescent. A soft ${avatarColor} aura surrounds it like a gentle halo, and it casts a faint shadow beneath it as if levitating. Its design is minimalist and adorable, reminiscent of a friendly creature from a video game — something between a magical spirit, a loyal companion, and a living nucleus of energy.`;
